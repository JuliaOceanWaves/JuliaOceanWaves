...
function HavelockGF(dx, dy, vv)
    GF = zeros(Complex{Float64}, 4)
    ...
    GF[1] = GF_Func_L0(hh, vv, dd, alpha, beta, rho) + GF_Func_W(hh, vv)
...
function evaluate_greens_func(source::Vector{Float64},image::Vector{Float64})
    dx = source[1]-image[1]
    dy = source[2]-image[2]
    vv = source[3]+image[3]
    dvv = source[3]-image[3]
    r = sqrt(dx^2 + dy^2 + dvv^2 ) #diff last term
    d = sqrt(dx^2 + dy^2 + vv^2 ) #sum
    #should we evaluate here..when r--> 0 this thing blows up
    return -(1/Complex(r))-(1/Complex(d)) .+ HavelockGF(dx,dy,vv)[1] #return [1] for free surface only.
end
